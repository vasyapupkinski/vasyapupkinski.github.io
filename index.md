...
"데이터의 품질이 곧 모델의 성능이다." 초급 프로젝트(Part 02) 당시 mAP 0.99 뒤에 숨겨진 **Data Leakage**를 철저한 EDA 분석으로 증명했고, 중급 프로젝트(Part 03)에서는 100여 건의 HWP 문서를 PDF로 정규화하는 파이프라인과 **임베딩용(Search) / 생성용(Context) 데이터를 분리하여 관리하는 이중화 파싱 로직**을 구축하며 데이터 정제에 몰입했습니다. 수치를 뽑기 전 '데이터의 무결성'을 확인하는 것이 제 엔지니어링의 시작입니다.
