---
layout: page
title: "Home"
home: true
---

# 🚀 LLM & Data-Centric AI Engineer

안녕하세요! 데이터의 본질적인 가치를 가장 중요하게 생각하고, 이를 이용해 LLM을 더 똑똑하게 만들고 서비스로 연결하는 일에 열정을 가진 **이승완**입니다.

## 🌟 Focus: LLM & Application
7개월간 머신러닝 기초부터 모델 서빙까지 AI 파이프라인 전 과정을 깊이 있게 탐구했습니다. 제가 집중하는 부분은 **LLM(Large Language Models)** 기반의 개발과 효율적인 **모델 서빙**입니다.

## ✍️ My 7-Month Journey
- **Phase 04. Model Serving & Agentic AI**: 에이전틱(Agentic) 워크플로우 탐구 및 실전 서빙 최적화. (Advanced Project)
- **Phase 03. Deep Learning & CV**: 비정상 데이터 상황에서의 논리적 데이터 증명. (Intermediate Project)
- **Phase 02. Machine Learning**: 'Garbage In, Garbage Out'을 몸소 체험하며 정교한 EDA 실천.
- **Phase 01. Python & Fundamentals**: 데이터 파이프라인의 기초 체력 다지기.

## 💡 Key Highlights: Experience & Perseverance

### 1️⃣ Data-Centric Philosophy (GIGO)
"데이터의 품질이 곧 모델의 성능이다." 단순히 소스코드를 고치는 것에 그치지 않고, 데이터셋 자체의 오류를 증명하거나(CV), 일주일간 EDA와 데이터 정제에만 몰입하여 모델의 토대를 닦는 과정을 거쳐왔습니다.

### 2️⃣ Agentic AI & Grit
고급 프로젝트에서는 팀원 대다수가 이탈하는 어려운 상황에서도 끝까지 남아 **'Agentic AI'** 구조를 구현해냈습니다. 비록 완벽한 완성도는 아니었지만, 최신 AI 트렌드를 적용하려는 도전 정신과 포기하지 않는 끈기를 배웠습니다.

### 3️⃣ Vision: World Monitor (Upcoming)
최근 글로벌 개발자의 사례에서 영감을 받아, 세상을 실시간으로 관찰하고 분석하는 **World Monitor** 프로젝트를 기획 중입니다. LLM과 실시간 데이터 기술을 결합하여 세상에 가치를 더하는 시스템을 만드는 것이 목표입니다.

---

### 📂 Explore My Work
| [Theory]({{ site.baseurl }}/1. Theory/) | [Milestones]({{ site.baseurl }}/2. Milestones/) | [Projects]({{ site.baseurl }}/3. Projects/) |
|:---:|:---:|:---:|
| 기술 블로그 & 이론 정리 | 7개월간의 교육 성실도 기록 | 주요 프로젝트 결과물 |
